...
technology that email receivers use to verify whether an email was legitimate
...
to add an extra record to your DNS server, in which you can
...
DKIM and SPF and that its mails might still be valid despite the mismatch.
...
many different parameters, and DMARC only makes it more complicated. 
The dashboard helps you using the concept of "sender domains".
A sender domain is a domain from which you intend to send out mail. 
If you use the dashboard to configure a sender domain, we automatically
create all the appropriate DNS records and private keys so that you can
start sending mail using that domain. You just have to copy the DNS records 
that were created by us to your own DNS server (or give them to your 
provider) and you are ready to go.


## Domains vs sub-domains

If you configure "yourdomain.com" as a sender domain, Copernica automatically
...
through the Marketing Suite. Be careful, though!

The moment that you update the DNS records for your domain, all mails that
you do not send through Copernica software are invalid. Although this does not
...
are passed through our software and are correctly signed and sent from the 
right servers.
...
"yourcompany.com" domain name, you can set up a sender
domain for a subdomain e.g. "newsletter.yourcompany.com". After this setup
you can use the Marketing Suite for messages with a _from_ address that 
...
you to experiment with deploying DMARC without having to worry about all your email
...
The Marketing Suite allows you to have this deployment done for you. You
can enter the percentage that you would like to have eventually, and the date on which you
want to have it deployed, and we will automatically update your
DNS records everyday to slowly get to this percentage.


## Processing reports

Email receivers send back daily reports in which they show
how many emails failed the DKIM, SPF or DMARC checks. These daily reports
are processed by the Marketing Suite and presented to you via the dashboard, under configuration -> sender domains -> manage domain -> DMARC reports. It allows
