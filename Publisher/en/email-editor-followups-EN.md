...
The {$mailing.trigger.subprofile} variable is mainly used in follow-up based on a new or modified subprofile, for example: "send a review mail to the profile two days after a new subprofile, for example an order, has been created". In this case, the profile is the destination of the email and the subprofile the trigger. This because the follow-up was started because of a new or modified subprofile. To show the subprofile data in a template, you normally use the [loadsubprofile](./loadprofile-and-loadsubprofile) tag. This retrieves the data of the last subprofile. However, it is possible that additional subprofiles have been created within the same profile during the waiting period. In this case you don't want to retrieve the values from the last subprofile, but from the subprofile on which the follow-up was originally started. This is possible with {$mailing.trigger.subprofile.FIELDNAME}.
