...
by simply starting up Yothalot processes on servers in your network. The map/reduce 
...
meaning that no data is lost when a server in the cluster crashes or
...
automatically requeues messages when servers or jobs crash, making it a ideal 
...
schedule your job. Writing map/reduce algorithms can not be simpler.


## Native API

If speed is however your thing, Yothalot also comes with a C++ API. With this
API you can write super fast C++ applications that run map/reduce jobs.
