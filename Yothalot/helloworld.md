...
Because Yothalot comes with a very simple [PHP API](Yothalot/phpapi),
we show how you can implement the WordCount map/reduce job in PHP. You simply start
by writing your own WordCount class that implements the [Yothalot\MapReduce](Yothalot/php-mapreduce) interface.
...
To send this [job](Yothalot/php-job "Job") to the Yothalot cluster,
you can write a second script that creates a "WordCount" instance, creates a
[connection](Yothalot/php-connection "Connection") to Yothalot, sends
this instance to the Yothalot master node, and specify on which data the
[job](Yothalot/php-job "Job") should run. This master node will send
...
Maybe you have noticed that we have used the [Yothalot\Path](Yothalot/php-path "Yothalot\Path")
class to find the full path name to the output file. Since you work with
[files on a distributed cluster](Yothalot/files "Files on the Yothalot cluster"),
...
can be a bit cumbersome then. The [Yothalot\Path](Yothalot/php-path "Yothalot\Path")
