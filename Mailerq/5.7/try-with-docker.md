...
## Installing
First, to install the 5.7 version of the MailerQ image, run 
```
docker pull mailerq/mailerq:5.7
```
...
```
docker run -e LICENSE_KEY=<your_license_key> -it mailerq/mailerq:5.7
```
...
```
docker run -v `pwd`/license.txt:/etc/mailerq/license.txt -it mailerq/mailerq:5.7
```
