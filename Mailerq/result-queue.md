...
As we explained above, the queues that are used to publish the results to are configured in the global configuration file. However, if you want to use different queues for specific emails, you can do so by [setting a property "queues"](/documentation/send-email#custom-queues "Curstom queues") in the JSON object.
