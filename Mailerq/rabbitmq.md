...
By including this option in your config file, you tell RabbitMQ that it is ok to 
...
MailerQ not only uses RabbitMQ to fetch the message that it is going to send, 
...
RabbitMQ server runs out of resources. This can especially happen in a production 
...
first checks if the queues and exchanges already exist. If they do not exist, 
MailerQ creates them. RabbitMQ allows to to set your queues and exchanges to be 
"durable". This means that the exchange or queue will continue to exist even when 
...
MailerQ outbox and inbox queues are mandatory, if you want to disable the other 
queues they can be simply left empty. 
...
uses an exchange with the name "mailerq". You also do not have to craete the 
exchange yourself. If the exchange does not exist MailerQ will tell RabbitMQ 
to create the exchange. An exchange is a very simple thing. On one side it 
...
MailerQ can also track refused messages using the `rabbitmq-reports`. Refused messages are messages
that MailerQ itself did not accept. This happens to messages delivered to MailerQ 
without first authenticating that are also not delivery reports. Authentication is only
...
mobil that have been successfully delivered to the success-queue, a copy of messages that 
