...
goes on between the server and the client, and it does not easily allow to
...
for the world wide web. To send an email your application sends an POST request to the 
SMTPeter API endpoint. To keep your emails and API keys secure, SMTPeter only accepts
mails via HTTPS. The API is not reachable over HTTP.
...
The above example shows how to send a message using traditional HTTP POST variables. But 
you may inject mails as JSON formatted. Be sure to set the 'content-type' header to 
application/json.

With every call to the REST API, you must pass your personalize API access token. To learn 
